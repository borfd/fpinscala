  ...
  "map2" >> {
    ...
    "should apply the function correctly" >> {
      Prop.forAll { long1: Long =>
        ...
        map2(nonNegativeEven, nonNegativeEven)(addition)(rng)._1 must_=== nonNegativeEven(rng.nextInt._2)._1 + nonNegativeEven(rng)._1
        map2(nonNegativeEven, int)(addition)(rng)._1 must_=== nonNegativeEven(rng)._1 + int(rng.nextInt._2)._1
      }
    }
  }
  ...
  "intsSeq" >> {
    ...
    "should return different lists for consecutive rngs" >> {
      Prop.forAll { (long: Long) =>
        val count = 0
        val rng = SimpleRNG(long)
        intsSeq(count)(rng)._1 must beEmpty
      }
    }
  }
